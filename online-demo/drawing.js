...
function draw(ctx, rotMat, result)
{
    ...
    if (result.mode === MEDIAPIPE_MODE.face_mesh) {
    ...
    } else if (result.mode === MEDIAPIPE_MODE.hands) {
        ...
        if (!! result.results.leftHandLandmarks) {
            const lcol = POSE_LANDMARK_COLORS[POSE_LANDMARK_INDICE.left_wrist];
        ...
        if (!! result.results.rightHandLandmarks) {
            const rcol = POSE_LANDMARK_COLORS[POSE_LANDMARK_INDICE.right_wrist];
...
function drawHandBones(ctx, rotMat, landmarks, rgba)
{
    ...
    const indice = HANDS_LANDMARK_INDICE;
	const w = ctx.canvas.width;
	const h = ctx.canvas.height;
...
{
    const color = POSE_LANDMARK_COLORS;
	const w = ctx.canvas.width;
	const h = ctx.canvas.height;
    const dotSize = 5;
...
function drawPoseBones(ctx, rotMat, landmarks)
{
    ...
    ctx.lineWidth = 10;
    const color = POSE_LANDMARK_COLORS;
    const indice = POSE_LANDMARK_INDICE;
	const w = ctx.canvas.width;
	const h = ctx.canvas.height;
    const dotSize = 5;
